...
def run_bowtie(con):
    ...
    for ii in x:
        ...
        sql = "select indexpath from SpeciesBowtieindex where speciesid=" + speciesid.__str__()
        cur.execute(sql)
        x = cur.fetchall()
        if x.__len__() == 0:
            print "\n. Error: I cannot find a Bowtie index file reference for the species " + speciesname
            print ". Check your BOWTIEINDEX option in the configuration file."
            exit()
        else:
            gpath = x[0][0]
        c += " -x " + gpath
